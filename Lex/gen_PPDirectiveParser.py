...
class Gen_PPDirectiveParser(ParserBase):
    ...
    def defined_macro_expression(self):
        ...
        self.restore(_z)
        if (
            (a := self.expect(TokenKind.DEFINED))
            and self.expect(TokenKind.L_PAREN)
            and (b := self.identifier())
            and self.expect(TokenKind.R_PAREN)
        ):
            return Defined(location=a.location, name=b.text)
        self.restore(_z)
    ...
    def has_include_expression(self):
        ...
        _z = self.save()
        if (
            (a := self.expect(TokenKind.HAS_INCLUDE))
            and self.expect(TokenKind.L_PAREN)
            and (b := self.header_name())
            and self.expect(TokenKind.R_PAREN)
        ):
            return HasInclude(
                location=a.location, filename=b.text, search_current_path=False
            )
        self.restore(_z)
        if (
            (a := self.expect(TokenKind.HAS_INCLUDE))
            and self.expect(TokenKind.L_PAREN)
            and (b := self.string_literal())
            and self.expect(TokenKind.R_PAREN)
        ):
            return HasInclude(
                location=a.location, filename=b.text[1:-1], search_current_path=True
            )
        self.restore(_z)
        if (
            (a := self.expect(TokenKind.HAS_INCLUDE))
    ...
    def has_embed_expression(self):
        ...
        _z = self.save()
        if (
            (a := self.expect(TokenKind.HAS_EMBED))
            and self.expect(TokenKind.L_PAREN)
            and (b := self.header_name())
            and ((c := self.embed_parameter_sequence()),)
            and self.expect(TokenKind.R_PAREN)
        ):
            return HasEmbed(
                location=a.location,
                filename=b.text,
                search_current_path=False,
                parameters=c,
            )
        self.restore(_z)
        if (
        ...
        if (
            ...
            and self.expect(TokenKind.L_PAREN)
            and (b := self.header_name())
            and ((embed_parameter_sequence := self.embed_parameter_sequence()),)
        ):
            return self.error("期待')'", self.curtoken().location)
        self.restore(_z)
        if (
    ...
    def has_c_attribute_express(self):
        ...
        _z = self.save()
        if (
            (a := self.expect(TokenKind.HAS_C_ATTRIBUTE))
            and self.expect(TokenKind.L_PAREN)
            and (b := self.attribute())
            and self.expect(TokenKind.R_PAREN)
        ):
        ...
        if (
            ...
            return self.error("期待')'", self.curtoken().location)
        self.restore(_z)
        if (a := self.expect(TokenKind.HAS_C_ATTRIBUTE)) and self.expect(
            TokenKind.L_PAREN
        ):
    ...
    def function_definition(self):
        ...
        if (
            ...
            and (d := self.function_body())
        ):
            return FunctionDef(
                attribute_specifiers=option(a, []),
                specifiers=b[0],
    ...
    def unlabeled_statement(self):
        ...
        self.restore(_z)
        if ((b := self.attribute_specifier_sequence()),) and (
            a := self.primary_block()
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
        if ((b := self.attribute_specifier_sequence()),) and (
            a := self.jump_statement()
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
        return None
    ...
    def label(self):
        ...
        _z = self.save()
        if (
            ((b := self.attribute_specifier_sequence()),)
            and (a := self.identifier())
            and self.expect(TokenKind.COLON)
        ):
            return LabelStmt(
                name=a.text, attribute_specifiers=option(b, []), location=a.location
            )
        self.restore(_z)
        if (
            ((b := self.attribute_specifier_sequence()),)
            and (a := self.expect(TokenKind.CASE))
            and (c := self.constant_expression())
            and self.expect(TokenKind.COLON)
        ):
            return CaseStmt(
                expr=c, attribute_specifiers=option(b, []), location=a.location
            )
        self.restore(_z)
        if (
            ((b := self.attribute_specifier_sequence()),)
            and (a := self.expect(TokenKind.DEFAULT))
            and self.expect(TokenKind.COLON)
        ):
            return DefaultStmt(attribute_specifiers=option(b, []), location=a.location)
        self.restore(_z)
        if (
            ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and (a := self.expect(TokenKind.CASE))
            and (constant_expression := self.constant_expression())
        ):
    ...
    def compound_statement(self):
        ...
        if (
            ...
            and ((a := self.block_item_list()),)
            and self.expect(TokenKind.R_BRACE)
        ):
            return CompoundStmt(items=option(a, []), location=b.location)
        self.restore(_z)
        if (a := self.expect(TokenKind.L_BRACE)) and (
            (block_item_list := self.block_item_list()),
        ):
    ...
    def selection_statement(self):
        ...
        if (
            ...
            and (a := self.expect(TokenKind.R_PAREN))
            and (secondary_block := self.secondary_block())
            and self.expect(TokenKind.ELSE)
        ):
    ...
    def iteration_statement(self):
        ...
        if (
            ...
            and ((c := self.expression()),)
            and self.expect(TokenKind.SEMI)
            and ((d := self.expression()),)
            and self.expect(TokenKind.R_PAREN)
            and (e := self.secondary_block())
        ):
            return ForStmt(
                init=b, condition_expr=c, increase_expr=d, body=e, location=a.location
            )
        self.restore(_z)
        if (
            (a := self.expect(TokenKind.FOR))
        ...
        if (
            ...
            and (secondary_block := self.secondary_block())
            and self.expect(TokenKind.WHILE)
            and (a := self.expect(TokenKind.L_PAREN))
        ):
            return self.error("后面期待一个表达式", a.location)
        self.restore(_z)
        if (
        ...
        if (
            self.expect(TokenKind.FOR)
            and self.expect(TokenKind.L_PAREN)
            and (declaration := self.declaration())
            and ((expression := self.expression()),)
            and self.expect(TokenKind.SEMI)
            and ((expression := self.expression()),)
            and (a := self.expect(TokenKind.R_PAREN))
    ...
    def expression(self):
        ...
        if (
            (a := self.expression())
            and (c := self.expect(TokenKind.COMMA))
            and (b := self.assignment_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.COMMA, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def assignment_expression(self):
        ...
        _z = self.save()
        if (
            (a := self.unary_expression())
            and (c := self.assignment_operator())
            and (b := self.assignment_expression())
        ):
    ...
    def conditional_expression(self):
        ...
        if (
            ...
            and (d := self.expect(TokenKind.QUESTION))
            and (b := self.expression())
            and self.expect(TokenKind.COLON)
            and (c := self.conditional_expression())
        ):
            return ConditionalOperator(
                condition_expr=a, true_expr=b, false_expr=c, location=d.location
            )
        self.restore(_z)
    ...
    def logical_OR_expression(self):
        ...
        if (
            (a := self.logical_OR_expression())
            and (c := self.expect(TokenKind.PIPEPIPE))
            and (b := self.logical_AND_expression())
        ):
    ...
    def logical_AND_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.AMPAMP))
            and (b := self.inclusive_OR_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.AND, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def inclusive_OR_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.PIPE))
            and (b := self.exclusive_OR_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.BITOR, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def exclusive_OR_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.CARET))
            and (b := self.AND_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.BITXOR, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def AND_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.AMP))
            and (b := self.equality_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.BITAND, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def equality_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.EQUALEQUAL))
            and (b := self.relational_expression())
        ):
            return BinaryOperator(op=BinOpKind.EQ, left=a, right=b, location=c.location)
        self.restore(_z)
        if (
            (a := self.equality_expression())
            and (c := self.expect(TokenKind.EXCLAIMEQUAL))
            and (b := self.relational_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.NEQ, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def relational_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.LESS))
            and (b := self.shift_expression())
        ):
            return BinaryOperator(op=BinOpKind.LT, left=a, right=b, location=c.location)
        self.restore(_z)
        if (
            (a := self.relational_expression())
            and (c := self.expect(TokenKind.GREATER))
            and (b := self.shift_expression())
        ):
        ...
        if (
            (a := self.relational_expression())
            and (c := self.expect(TokenKind.LESSEQUAL))
            and (b := self.shift_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.LTE, left=a, right=b, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.relational_expression())
            and (c := self.expect(TokenKind.GREATEREQUAL))
            and (b := self.shift_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.GTE, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def shift_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.LESSLESS))
            and (b := self.additive_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.LSHIFT, left=a, right=b, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.shift_expression())
            and (c := self.expect(TokenKind.GREATERGREATER))
            and (b := self.additive_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.RSHIFT, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def additive_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.PLUS))
            and (b := self.multiplicative_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.ADD, left=a, right=b, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.additive_expression())
            and (c := self.expect(TokenKind.MINUS))
            and (b := self.multiplicative_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.SUB, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def multiplicative_expression(self):
        ...
        if (
            ...
            and (c := self.expect(TokenKind.STAR))
            and (b := self.cast_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.MUL, left=a, right=b, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.multiplicative_expression())
            and (c := self.expect(TokenKind.SLASH))
            and (b := self.cast_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.DIV, left=a, right=b, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.multiplicative_expression())
            and (c := self.expect(TokenKind.PERCENT))
            and (b := self.cast_expression())
        ):
            return BinaryOperator(
                op=BinOpKind.MOD, left=a, right=b, location=c.location
            )
        self.restore(_z)
    ...
    def unary_expression(self):
        ...
        if (a := self.expect(TokenKind.PLUSPLUS)) and (b := self.unary_expression()):
            return UnaryOperator(
                op=UnaryOpKind.PREFIX_INC, operand=b, location=a.location
            )
        self.restore(_z)
        if (a := self.expect(TokenKind.MINUSMINUS)) and (b := self.unary_expression()):
            return UnaryOperator(
                op=UnaryOpKind.PREFIX_DEC, operand=b, location=a.location
            )
        self.restore(_z)
    ...
    def postfix_expression(self):
        ...
        if (
            (a := self.postfix_expression())
            and (c := self.expect(TokenKind.L_PAREN))
            and ((b := self.argument_expression_list()),)
            and self.expect(TokenKind.R_PAREN)
        ):
        ...
        if (
            (a := self.postfix_expression())
            and (c := self.expect(TokenKind.PERIOD))
            and (b := self.identifier())
        ):
            return MemberRef(
                target=a, member_name=b.text, is_arrow=False, location=c.location
            )
        self.restore(_z)
        if (
            (a := self.postfix_expression())
            and (c := self.expect(TokenKind.ARROW))
            and (b := self.identifier())
        ):
            return MemberRef(
                target=a, member_name=b.text, is_arrow=True, location=c.location
            )
        self.restore(_z)
        if (a := self.postfix_expression()) and (b := self.expect(TokenKind.PLUSPLUS)):
            return UnaryOperator(
                op=UnaryOpKind.POSTFIX_INC, operand=a, location=b.location
            )
        self.restore(_z)
        if (a := self.postfix_expression()) and (
            b := self.expect(TokenKind.MINUSMINUS)
        ):
            return UnaryOperator(
                op=UnaryOpKind.POSTFIX_DEC, operand=a, location=b.location
            )
        self.restore(_z)
    ...
    def argument_expression_list(self):
        ...
        _z = self.save()
        if (
            (a := self.argument_expression_list())
    ...
    def compound_literal(self):
        ...
        if (
            self.expect(TokenKind.L_PAREN)
            and (storage_class_specifiers := self.storage_class_specifiers())
            and (type_name := self.type_name())
        ):
            return self.error("前面应有')'", self.curtoken().location)
        self.restore(_z)
    ...
    def storage_class_specifiers(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.storage_class_specifiers()) and (
            b := self.storage_class_specifier()
        ):
            return a + [b]
        self.restore(_z)
    ...
    def generic_selection(self):
        ...
        if (
            ...
            and (b := self.assignment_expression())
            and self.expect(TokenKind.COMMA)
            and (c := self.generic_assoc_list())
            and self.expect(TokenKind.R_PAREN)
        ):
            return GenericSelection(
                controling_expr=b, assoc_list=c, location=a.location
            )
        self.restore(_z)
        if (
            self.expect(TokenKind._GENERIC)
            and self.expect(TokenKind.L_PAREN)
            and (assignment_expression := self.assignment_expression())
            and self.expect(TokenKind.COMMA)
            and (generic_assoc_list := self.generic_assoc_list())
        ):
            return self.error("前面应有')'", self.curtoken().location)
        self.restore(_z)
        if (
            ...
            and self.expect(TokenKind.L_PAREN)
            and (assignment_expression := self.assignment_expression())
            and (a := self.expect(TokenKind.COMMA))
        ):
    ...
    def generic_assoc_list(self):
        ...
        _z = self.save()
        if (
            (a := self.generic_assoc_list())
            and self.expect(TokenKind.COMMA)
            and (b := self.generic_association())
        ):
            return a + [b]
        self.restore(_z)
    ...
    def generic_association(self):
        ...
        if (
            ...
            return GenericAssociation(
                type_name=a, expr=b, is_default=False, location=a.location
            )
        self.restore(_z)
        if (
    ...
    def declaration(self):
        ...
        if (
            (a := self.attribute_specifier_sequence())
            and (b := self.declaration_specifiers())
            and (c := self.init_declarator_list())
            and (d := self.expect(TokenKind.SEMI))
        ):
            return DeclStmt(
        ...
        self.restore(_z)
        if (
            (a := self.declaration_specifiers())
            and ((b := self.init_declarator_list()),)
            and (c := self.expect(TokenKind.SEMI))
        ):
            return DeclStmt(
                attribute_specifiers=[],
                specifiers=a[0],
                specifier_attributes=a[1],
                declarators=option(b, []),
                location=c.location,
            )
        self.restore(_z)
        ...
        if (
            (attribute_specifier_sequence := self.attribute_specifier_sequence())
            and (declaration_specifiers := self.declaration_specifiers())
            and (init_declarator_list := self.init_declarator_list())
        ):
            return self.error("前面缺少一个';'", self.curtoken().location)
        self.restore(_z)
        if (attribute_specifier_sequence := self.attribute_specifier_sequence()) and (
            declaration_specifiers := self.declaration_specifiers()
        ):
            return self.error(
                "如果有属性列表就应该有初始化列表", self.curtoken().location
            )
        self.restore(_z)
        if (declaration_specifiers := self.declaration_specifiers()) and (
    ...
    def declaration_specifiers(self):
        ...
        if (a := self.declaration_specifier()) and (b := self.declaration_specifiers()):
            return ([a] + b[0], b[1])
        self.restore(_z)
        if (a := self.declaration_specifier()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return ([a], option(b, []))
        self.restore(_z)
    ...
    def init_declarator_list(self):
        ...
        _z = self.save()
        if (
            (a := self.init_declarator_list())
            and self.expect(TokenKind.COMMA)
            and (b := self.init_declarator())
        ):
            return a + [b]
        self.restore(_z)
    ...
    def init_declarator(self):
        ...
        if (
            (a := self.declarator())
            and (c := self.expect(TokenKind.EQUAL))
    ...
    def attribute_declaration(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.attribute_specifier_sequence()) and (
            b := self.expect(TokenKind.SEMI)
        ):
    ...
    def storage_class_specifier(self):
        ...
        if a := self.expect(TokenKind.AUTO):
            return StorageClass(
                specifier=StorageClassSpecifier.AUTO, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.CONSTEXPR):
            return StorageClass(
                specifier=StorageClassSpecifier.CONSTEXPR, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.EXTERN):
            return StorageClass(
                specifier=StorageClassSpecifier.EXTERN, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.REGISTER):
            return StorageClass(
                specifier=StorageClassSpecifier.REGISTER, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.STATIC):
            return StorageClass(
                specifier=StorageClassSpecifier.STATIC, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.THREAD_LOCAL):
            return StorageClass(
                specifier=StorageClassSpecifier.THREAD_LOCAL, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.TYPEDEF):
            return StorageClass(
                specifier=StorageClassSpecifier.TYPEDEF, location=a.location
            )
        self.restore(_z)
        return None
    ...
    def struct_or_union_specifier(self):
        ...
        _z = self.save()
        if (
            (a := self.struct_or_union())
            and ((b := self.attribute_specifier_sequence()),)
            and ((c := self.identifier()),)
            and self.expect(TokenKind.L_BRACE)
            and (d := self.member_declaration_list())
            and self.expect(TokenKind.R_BRACE)
        ):
            return RecordDecl(
                struct_or_union=a.text,
                attribute_specifiers=option(b, []),
        ...
        self.restore(_z)
        if (
            (struct_or_union := self.struct_or_union())
            and ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and ((identifier := self.identifier()),)
            and self.expect(TokenKind.L_BRACE)
            and (member_declaration_list := self.member_declaration_list())
        ):
        ...
        if (
            ...
            and ((identifier := self.identifier()),)
            and self.expect(TokenKind.L_BRACE)
            and (a := self.expect(TokenKind.R_BRACE))
        ):
        ...
        if (
            ...
            and ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and ((identifier := self.identifier()),)
            and (a := self.expect(TokenKind.L_BRACE))
        ):
            return self.error("后面缺少成员声明列表", a.location)
        self.restore(_z)
        if (struct_or_union := self.struct_or_union()) and (
            (attribute_specifier_sequence := self.attribute_specifier_sequence()),
        ):
            return self.error(
                "匿名的记录声明必须有成员声明列表", self.curtoken().location
            )
        self.restore(_z)
        return None
    ...
    def member_declaration(self):
        ...
        if (
            ((a := self.attribute_specifier_sequence()),)
            and (b := self.specifier_qualifier_list())
            and ((c := self.member_declarator_list()),)
            and (d := self.expect(TokenKind.SEMI))
        ):
            return FieldDecl(
                attribute_specifiers=option(a, []),
        ...
        if (
            ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and (specifier_qualifier_list := self.specifier_qualifier_list())
            and ((member_declarator_list := self.member_declarator_list()),)
        ):
            return self.error("前面缺少一个';'", self.curtoken().location)
        self.restore(_z)
        if attribute_specifier_sequence := self.attribute_specifier_sequence():
    ...
    def specifier_qualifier_list(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.type_specifier_qualifier()) and (
    ...
    def member_declarator_list(self):
        ...
        _z = self.save()
        if (
            (a := self.member_declarator_list())
            and self.expect(TokenKind.COMMA)
            and (b := self.member_declarator())
        ):
            return a + [b]
        self.restore(_z)
    ...
    def member_declarator(self):
        ...
        if (
            ((a := self.declarator()),)
            and (c := self.expect(TokenKind.COLON))
            and (b := self.constant_expression())
        ):
    ...
    def enum_specifier(self):
        ...
        if (
            ...
            and ((b := self.attribute_specifier_sequence()),)
            and ((c := self.identifier()),)
            and ((d := self.enum_type_specifier()),)
            and self.expect(TokenKind.L_BRACE)
            and (e := self.enumerator_list())
            and (self.expect(TokenKind.COMMA),)
            and self.expect(TokenKind.R_BRACE)
        ):
            return EnumDecl(
        ...
        if (
            ...
            and ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and ((identifier := self.identifier()),)
            and ((enum_type_specifier := self.enum_type_specifier()),)
            and self.expect(TokenKind.L_BRACE)
            and (enumerator_list := self.enumerator_list())
            and (self.expect(TokenKind.COMMA),)
        ):
            return self.error("前面缺少一个右大括号", self.curtoken().location)
        self.restore(_z)
    ...
    def enumerator_list(self):
        ...
        _z = self.save()
        if (
            (a := self.enumerator_list())
            and self.expect(TokenKind.COMMA)
            and (b := self.enumerator())
        ):
            return a + [b]
        self.restore(_z)
    ...
    def enumerator(self):
        ...
        self.restore(_z)
        if (a := self.enumeration_constant()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return Enumerator(
                name=a.text,
        ...
        if (
            ...
            and ((attribute_specifier_sequence := self.attribute_specifier_sequence()),)
            and (a := self.expect(TokenKind.EQUAL))
        ):
            return self.error("后面缺少一个表达式", a.location)
        self.restore(_z)
        return None
    ...
    def atomic_type_specifier(self):
        ...
        self.restore(_z)
        if (
            self.expect(TokenKind._ATOMIC)
            and self.expect(TokenKind.L_PAREN)
            and (a := self.type_name())
        ):
            return self.error("后面缺少')'", a.location)
        self.restore(_z)
        return None
    ...
    def typeof_specifier(self):
        ...
        if (
            ...
            and self.expect(TokenKind.L_PAREN)
            and (typeof_specifier_argument := self.typeof_specifier_argument())
        ):
        ...
        self.restore(_z)
        if (
            self.expect(TokenKind.TYPEOF_UNQUAL)
            and self.expect(TokenKind.L_PAREN)
            and (typeof_specifier_argument := self.typeof_specifier_argument())
        ):
            return self.error("前面缺少')'", self.curtoken().location)
        self.restore(_z)
        if self.expect(TokenKind.TYPEOF_UNQUAL) and (
            a := self.expect(TokenKind.L_PAREN)
        ):
            return self.error("后面期待一个类型", a.location)
        self.restore(_z)
    ...
    def type_qualifier(self):
        ...
        if a := self.expect(TokenKind.RESTRICT):
            return TypeQualifier(
                qualifier=TypeQualifierKind.RESTRICT, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind.VOLATILE):
            return TypeQualifier(
                qualifier=TypeQualifierKind.VOLATILE, location=a.location
            )
        self.restore(_z)
        if a := self.expect(TokenKind._ATOMIC):
            return TypeQualifier(
                qualifier=TypeQualifierKind._ATOMIC, location=a.location
            )
        self.restore(_z)
        return None
    ...
    def direct_declarator(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.array_declarator()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
        if (a := self.function_declarator()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
    ...
    def array_declarator(self):
        ...
        _z = self.save()
        if (
            (a := self.direct_declarator())
            and (d := self.expect(TokenKind.L_SQUARE))
            and (b := self.type_qualifier_list())
        ...
        if (
            ...
            and (type_qualifier_list := self.type_qualifier_list())
            and self.expect(TokenKind.STATIC)
            and (c := self.assignment_expression())
        ):
        ...
        if (
            ...
            and self.expect(TokenKind.L_SQUARE)
            and (type_qualifier_list := self.type_qualifier_list())
            and (a := self.expect(TokenKind.STATIC))
        ):
            return self.error("后面缺少一个表达式", a.location)
        self.restore(_z)
        if (
    ...
    def function_declarator(self):
        ...
        if (
            (a := self.direct_declarator())
            and (c := self.expect(TokenKind.L_PAREN))
            and ((b := self.parameter_type_list()),)
            and self.expect(TokenKind.R_PAREN)
        ):
            return FunctionDeclarator(
                declarator=a,
                parameters=option(b, [[], False])[0],
                has_varparam=option(b, [[], False])[1],
                location=c.location,
            )
        self.restore(_z)
        if (
            (direct_declarator := self.direct_declarator())
            and self.expect(TokenKind.L_PAREN)
            and ((parameter_type_list := self.parameter_type_list()),)
        ):
            return self.error("前面缺少')'", self.curtoken().location)
        self.restore(_z)
        return None
    ...
    def pointer(self):
        ...
        _z = self.save()
        if (
            (a := self.expect(TokenKind.STAR))
            and ((c := self.attribute_specifier_sequence()),)
            and ((d := self.type_qualifier_list()),)
            and ((e := self.pointer()),)
        ):
            return PointerDeclarator(
                declarator=e,
                attribute_specifiers=option(c, []),
                qualifiers=option(d, []),
                location=a.location,
            )
        self.restore(_z)
    ...
    def parameter_type_list(self):
        ...
        if self.expect(TokenKind.ELLIPSIS):
            return ([], True)
        self.restore(_z)
        if (parameter_list := self.parameter_list()) and (
            a := self.expect(TokenKind.COMMA)
        ):
            return self.error("后面期待一个'...'", a.location)
        self.restore(_z)
        return None
    ...
    def parameter_list(self):
        ...
        if (
            (a := self.parameter_list())
            and self.expect(TokenKind.COMMA)
            and (b := self.parameter_declaration())
        ):
            return a + [b]
        self.restore(_z)
    ...
    def parameter_declaration(self):
        ...
        if (
            ((a := self.attribute_specifier_sequence()),)
            and (b := self.declaration_specifiers())
            and ((c := self.abstract_declarator()),)
        ):
            return ParamDecl(
                attribute_specifiers=option(a, []),
                specifiers=b[0],
    ...
    def type_name(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.specifier_qualifier_list()) and (
            (b := self.abstract_declarator()),
        ):
            return TypeName(
                specifiers=a[0],
                specifier_attributes=a[1],
    ...
    def direct_abstract_declarator(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (a := self.array_abstract_declarator()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
        if (a := self.function_abstract_declarator()) and (
            (b := self.attribute_specifier_sequence()),
        ):
            return assign(a, "attribute_specifiers", option(b, []))
        self.restore(_z)
    ...
    def array_abstract_declarator(self):
        ...
        _z = self.save()
        if (
            ((a := self.direct_abstract_declarator()),)
            and (d := self.expect(TokenKind.L_SQUARE))
            and self.expect(TokenKind.STATIC)
            and ((b := self.type_qualifier_list()),)
            and (c := self.assignment_expression())
    ...
    def function_abstract_declarator(self):
        ...
        if (
            ((a := self.direct_abstract_declarator()),)
            and (d := self.expect(TokenKind.L_PAREN))
            and ((b := self.parameter_type_list()),)
            and self.expect(TokenKind.R_PAREN)
        ):
            return FunctionDeclarator(
    ...
    def braced_initializer(self):
        ...
        _z = self.save()
        if (
            (a := self.expect(TokenKind.L_BRACE))
            and (b := self.initializer_list())
            and (self.expect(TokenKind.COMMA),)
            and self.expect(TokenKind.R_BRACE)
        ):
        ...
        if (
            ...
            and (initializer_list := self.initializer_list())
            and (self.expect(TokenKind.COMMA),)
        ):
            return self.error("前面缺少一个右大括号", self.curtoken().location)
        self.restore(_z)
    ...
    def initializer_list(self):
        ...
        _z = self.save()
        if (
            (a := self.initializer_list())
            and self.expect(TokenKind.COMMA)
            and ((b := self.designation()),)
            and (c := self.initializer())
        ):
    ...
    def static_assert_declaration(self):
        ...
        self.restore(_z)
        if (
            self.expect(TokenKind.STATIC_ASSERT)
            and self.expect(TokenKind.L_PAREN)
            and (constant_expression := self.constant_expression())
            and self.expect(TokenKind.COMMA)
            and (string_literal := self.string_literal())
            and (d := self.expect(TokenKind.R_PAREN))
        ):
        ...
        if (
            ...
            return self.error("前面缺少')'", self.curtoken().location)
        self.restore(_z)
        if self.expect(TokenKind.STATIC_ASSERT) and (
            a := self.expect(TokenKind.L_PAREN)
        ):
            return self.error("后面期待一个类型", a.location)
        self.restore(_z)
    ...
    def attribute_specifier(self):
        ...
        if (
            ...
            and self.expect(TokenKind.L_SQUARE)
            and (attribute_list := self.attribute_list())
            and (a := self.expect(TokenKind.R_SQUARE))
        ):
    ...
    def attribute_token(self):
        ...
        if a := self.attribute_prefixed_token():
            return Attribute(
                prefix_name=a[0].text, name=a[1].text, location=begin_location
            )
        self.restore(_z)
    ...
    def attribute_prefixed_token(self):
        ...
        _z = self.save()
        if (
            (a := self.attribute_prefix())
            and self.expect(TokenKind.COLONCOLON)
            and (b := self.identifier())
        ):
            return (a, b)
        self.restore(_z)
        if (attribute_prefix := self.attribute_prefix()) and (
            a := self.expect(TokenKind.COLONCOLON)
        ):
    ...
    def attribute_argument_clause(self):
        ...
        if (
            ...
            return option(a, [])
        self.restore(_z)
        if self.expect(TokenKind.L_PAREN) and (
            (balanced_token_sequence := self.balanced_token_sequence()),
        ):
            return self.error("前面缺少')'", self.curtoken().location)
        self.restore(_z)
        return None
    ...
    def control_line(self):
        ...
        if (
            ...
            and (a := self.expect(TokenKind.INCLUDE))
            and (b := self.header_name())
            and ((pp_tokens := self.pp_tokens()),)
            and (new_line := self.new_line())
        ):
            return Include(
                location=a.location, filename=b.text, search_current_path=False
            )
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.EMBED))
            and (b := self.header_name())
            and ((c := self.embed_parameter_sequence()),)
            and ((pp_tokens := self.pp_tokens()),)
            and (new_line := self.new_line())
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.DEFINE))
            and (b := self.identifier())
            and (lparen := self.lparen())
            and (c := self.identifier_list())
            and self.expect(TokenKind.COMMA)
            and self.expect(TokenKind.ELLIPSIS)
            and self.expect(TokenKind.R_PAREN)
            and ((d := self.replacement_list()),)
            and (new_line := self.new_line())
        ):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.UNDEF))
            and (b := self.identifier())
            and (new_line := self.new_line())
        ):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.LINE))
            and (b := self.integer_constant())
            and ((c := self.string_literal()),)
            and (new_line := self.new_line())
        ):
            return LineDirecvtive(
                location=a.location, lineno=int(b.text), filename=c if c != None else ""
            )
        self.restore(_z)
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ERROR))
            and ((b := self.pp_tokens()),)
            and (new_line := self.new_line())
        ):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.WARNING))
            and ((b := self.pp_tokens()),)
            and (new_line := self.new_line())
        ):
            return WarningDirecvtive(
                location=a.location, messages=b if b != None else []
            )
        self.restore(_z)
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.PRAGMA))
            and ((b := self.pp_tokens()),)
            and (new_line := self.new_line())
        ):
        ...
        if (
            self.expect(TokenKind.HASH)
            and self.expect(TokenKind.INCLUDE)
            and (header_name := self.header_name())
            and ((pp_tokens := self.pp_tokens()),)
        ):
        ...
        if (
            self.expect(TokenKind.HASH)
            and self.expect(TokenKind.EMBED)
            and (header_name := self.header_name())
            and ((embed_parameter_sequence := self.embed_parameter_sequence()),)
            and ((pp_tokens := self.pp_tokens()),)
        ):
    ...
    def if_section(self):
        ...
        if (
            (a := self.if_group())
            and ((b := self.elif_groups()),)
            and ((c := self.else_group()),)
            and (endif_line := self.endif_line())
        ):
    ...
    def if_group(self):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.IF))
            and (b := self.constant_expression())
            and (new_line := self.new_line())
            and ((c := self.group()),)
        ):
            return IfDirecvtive(
                ...
                expr=b,
                group=c if c != None else [],
            )
        self.restore(_z)
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.IFDEF))
            and (b := self.identifier())
            and (new_line := self.new_line())
            and ((c := self.group()),)
        ):
            return IfdefDirecvtive(
                location=a.location,
                symtab=self.tokengen.symtab,
                name=b.text,
                group=c if c != None else [],
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.IFNDEF))
            and (b := self.identifier())
            and (new_line := self.new_line())
            and ((c := self.group()),)
        ):
            return IfndefDirecvtive(
                location=a.location,
                symtab=self.tokengen.symtab,
                name=b.text,
                group=c if c != None else [],
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ELIF))
            and (b := self.constant_expression())
        ):
            return self.error("缺少换行符", self.curtoken().location)
        self.restore(_z)
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ELIFDEF))
            and (b := self.identifier())
        ):
            return self.error("缺少换行符", self.curtoken().location)
        self.restore(_z)
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ELIFNDEF))
            and (b := self.identifier())
        ):
            return self.error("缺少换行符", self.curtoken().location)
        self.restore(_z)
    ...
    def else_group(self):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ELSE))
            and (new_line := self.new_line())
            and ((b := self.group()),)
        ):
    ...
    def endif_line(self):
        ...
        if (
            self.expect(TokenKind.HASH)
            and (a := self.expect(TokenKind.ENDIF))
            and (new_line := self.new_line())
        ):
