...
class Gen_BuilderParser(ParserBase):
    ...
    def grammar(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if ((header := self.header()),) and (rules := self.rules()):
            return Grammar(header=header.text if header != None else None, rules=rules, location=begin_location)
        self.restore(_z)
        return None
    ...
    def rules(self):
        begin_location = self.curtoken().location
        _z = self.save()
        if (rules := self.rules()) and (rule := self.rule()):
            return rules + [rule]
        self.restore(_z)
        if (rule := self.rule()):
            return [rule]
    ...
    def alt(self):
        ...
        if (items := self.items()) and ((action := self.action()),):
            return Alt(items=items, location=begin_location, action=action.text if action != None else None)
        self.restore(_z)
        return None
